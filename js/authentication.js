...
function getPoolData() {
    return {
        UserPoolId: 'us-east-1_XqotfSoiz',
        ClientId: '42kiqgg8cpcnek362157td482r'
    };
}
...
function initAWSConfig() {
    ...
    AWS.config.region = 'us-east-1'; // Set to your AWS service region

    AWS.config.credentials = new AWS.CognitoIdentityCredentials({
        IdentityPoolId: 'us-east-1:549828d7-1f7b-46a6-a334-c836891e160c'
    });

    // Explicitly get credentials from AWS
    AWS.config.credentials.get(function (err) {
        ...
        // Now you can safely make your AWS service calls
    });
}
...
function wrapCallback(callback) {
    return {
        ...
        onSuccess: (result) => {
            callback(null, result);
            onSuccessSignIn(); // This line ensures that the onSuccessSignIn function is called on successful sign-in
        }
    };
}
